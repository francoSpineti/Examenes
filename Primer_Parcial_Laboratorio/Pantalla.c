...
static int pantalla_buscarIndicePorId(Pantalla* arrayPantalla,int longitud,int id);
...
int pantalla_mostrarPantalla(Pantalla* arrayPantalla, int longitud)
{
    ...
    if(arrayPantalla != NULL && longitud > 0)
    {
        for(i=0; i < longitud ; i++)
        {
            if(arrayPantalla[i].flag == PANTALLA_OCUPADA)
            {
                printf("\n \n \n ID: %d \n - Nombre: %s \n - Direccion: %s \n - Tipo: %s \n - Precio: %.2f \n",arrayPantalla[i].idPantalla,arrayPantalla[i].nombre,arrayPantalla[i].direccion,arrayPantalla[i].tipoPantalla,arrayPantalla[i].precio);
                retorno = 0;
            }
        }
    }
